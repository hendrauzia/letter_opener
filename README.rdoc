...
There are cases when you don't need to open the rendered mail automatically using launchy, instead you may want to investigate the content of the rendered mail using other methods like capybara or other form of automation. You can do this by add the following configuration:

    LetterOpener::DeliveryMethod.render_only = true

And you can retrieve the escaped url of the last rendered email using the following method:

    LetterOpener::DeliveryMethod.last_rendered_mail_url

If you wish to have the original path of the rendered mail, you can access it using the following method:

    LetterOpener::DeliveryMethod.last_rendered_mail_path
